...
def train_classifier(nbr):
    #dataset_dir = "C:/Users/raja/PycharmProjects/FlaskOpencv_FaceRecognition/dataset"
    dataset_dir = "dataset"
...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< Face Recognition >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
def face_recognition(group_id, attendancetime, attendanceduration, random_attendance_id, user_id):  # generate frame by frame from camera
    def draw_boundary(img, classifier, scaleFactor, minNeighbors, color, text, clf):
        gray_image = cv2.cvtColor(img, cv2.COLOR_BGR2GRAY)
        features = classifier.detectMultiScale(gray_image, scaleFactor, minNeighbors)
        ...
        for (x, y, w, h) in features:
            ...
            if confidence > 70 and not justscanned:
                ...
                #select accs_added, DATE_ADD(NOW(), INTERVAL + 10 MINUTE) from accs_hist where accs_added < DATE_ADD(NOW(), INTERVAL 10 MINUTE) AND group_id = '7' AND accs_prsn = '2'
                ...
                rowcount = row[0]
                if rowcount > 0:
                    #cv2.putText(img, pname + ', You already marked your attendance ', (x - 10, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6,
                ...
                if int(cnt) == 30:
                    ...
                    #print(atime)
                    #datetime_object = datetime.strptime(atime, '%m/%d/%y %H:%M:%S')
                    #group_id = session.group_id #session['group_id']

                    mycursor.execute("insert into accs_hist (accs_date, accs_prsn, group_id, accs_added, random_attendance_id) values('" + str(
                    date.today()) + "', '" + pnbr + "', '" + str(group_id) + "', '" + str(atime) + "', '" + str(random_attendance_id) + "')")
                    mydb.commit()
                ...
                if not justscanned:
                    ...
                    cv2.putText(img, 'You already marked your attendance', (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 0, 255), 2, cv2.LINE_AA)
    ...
    #faceCascade = cv2.CascadeClassifier(
     #   "C:/Users/raja/PycharmProjects/FlaskOpencv_FaceRecognition/resources/haarcascade_frontalface_default.xml")
    faceCascade = cv2.CascadeClassifier("resources/haarcascade_frontalface_default.xml")
    clf = cv2.face.LBPHFaceRecognizer_create()
    clf.read("classifier.xml")
...
def userlist():
    ...
    mycursor = mydb.cursor()
    data1 = ""
    #mycursor.execute("select * from users where user_role!='admin'")
    mycursor.execute("select DISTINCT(b.img_person),a.* from users a LEFT JOIN img_dataset b ON a.id=b.img_person where user_role!='admin'")
    data = mycursor.fetchall()
...
def grouprequest():
    ...
    group_id = request.args.get('group_id')
    action = request.args.get('action')
    groupadmin = request.args.get('groupadmin')
    groupname = request.args.get('groupname')
    msg = "yes"
    userrole = session['user_role']
    if action == 'invitelink' and userrole != 'admin':
        ...
        if account:
            ...
            if accounts:
                #return render_template("grouprequest.html", group_id=group_id,groupadmin=groupadmin,groupname=groupname)
    ...
    return render_template('grouprequest.html', msg=msg, group_id=group_id,groupadmin=groupadmin,groupname=groupname)
    ...
    if action == 'view_members':
        ...
        mycursor.execute(
            "SELECT users.first_name,users.last_name,users.user_role FROM `join_groups` left JOIN groups ON join_groups.group_id=groups.id left JOIN users ON join_groups.user_id=users.id WHERE join_groups.group_id='" + str(
                group_id) + "'")
        data1 = mycursor.fetchall()
    return render_template('grouplist.html', data=data, data1=data1, groupadmin=groupadmin, groupname=groupname)
...
def agrouplist():
    ...
    return render_template('agrouplist.html', data=data, data1=data1, groupadmin=groupadmin, groupname=groupname, group_id=group_id, data2=data2, data3=data3, data4=data4)
